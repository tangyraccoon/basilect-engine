"""Surface ranked artist pairs by critic similarity.

Mirrors discover.py but works with critic_similarity.npy.
"""
...
def main():
    ...
    if not ids_path.exists() or not sim_path.exists():
        print(f"Error: critic similarity data not found. Run compute_critics.py first.")
        return
    ...
    print(f"\n{'=' * 50}")
    print(f"  CRITIC SIMILARITY RANKING ({len(pairs)} pairs)")
    print(f"{'=' * 50}")
    for rank, p in enumerate(pairs, 1):
        print(f"  {rank:2d}. {p['a']} × {p['b']}  —  {p['score']:.3f}")

    out_path = DATA_DIR / "critic_discoveries.json"
